	...
	<view class="wrap">
		...
		<view class="content">
			...
			<view class="login-title">
				<view>
					<text :class="type==0?'active':''" @click="type = 0">登录</text>
				</view>
				<view>
					<text :class="type==1?'active':''" @click="type = 1">注册</text>
				</view>
			</view>
			<view v-if="type===0">
				<view class="u-border-bottom">
					<u-input type="text" v-model="phone" maxlength="11" placeholder="请输入手机号"/>
				</view>
				<view class="u-border-bottom">
					<u-input type="password" v-model="password" placeholder="请输入密码"/>
				</view>
				<view class="u-border-bottom">
					<u-input type="text" v-model="captcha" maxlength="11" placeholder="请输入图形验证码"/>
					<u-image class="captchaImg" width="240rpx" height="72rpx" :fade="false" :src="imgCaptcha.img"
							 @click="getImgCaptcha()"/>
				</view>
				<view class="hint">
					<label class="radio">
						...
						我已阅读
						<text class="link" @click="$u.route('/pages/user/protocol')">服务协议</text>
						和
						<text class="link" @click="$u.route('/pages/user/protocol')">隐私政策</text>
					</label>
				</view>
				<!-- #ifdef MP-WEIXIN -->
				<button @tap="wxlogin" :style="[inputStyle]" style="margin-bottom: 20px;" class="getCaptcha"
						open-type="getPhoneNumber" @getphonenumber="getphonenumberWx">微信账号一键登录
				</button>
				<!-- #endif -->
				<!-- #ifdef MP-ALIPAY -->
				<button @tap="zfblogin" :style="[inputStyle]" style="margin-bottom: 20px;" class="getCaptcha"
						open-type="getPhoneNumber" @getphonenumber="getPhoneNumberZFB">支付宝账号一键登录
				</button>
				<!-- #endif -->
				...
				<view class="password_register">
					<text @click="$u.route('/pages/user/register')">注册账号</text>
					<text @click="$u.route('/pages/login/forgetPassword')">忘记密码？</text>
				</view>
			</view>
			...
			<view v-else-if="type===1">
				...
				<view class="u-border-bottom">
					<u-input type="password" v-model="password1" placeholder="输入密码"/>
				</view>
				<view class="u-border-bottom">
				...
				<view class="u-border-bottom">
					<u-input type="text" v-model="yzm" placeholder="请输入短信验证码"/>
					<u-button class="code-btn" @click="getPhoneCaptcha" :disabled="btnBool" type="default" :hair-line="false"
				...
				<view class="hint">
					<label class="radio">
						<u-checkbox v-model="isAgree" shape="circle"/>
						我已阅读
						<text class="link" @click="toUserAgreement">服务协议</text>
						和
